...
import type { ApiListingImage } from "@/types";
...
export const listingImagesApi = {
  getAll: (): Promise<ApiListingImage[]> => api.get<ApiListingImage[]>("/listing-images"),
  ...
  /** Get URL to display an image directly in <img> tags */
  ...
  upload: async (
    ...
    if (!ALLOWED.includes(file.type)) throw new Error("Invalid file type. Allowed: JPEG, PNG, GIF, WebP, BMP");
    if (file.size > 10 * 1024 * 1024) throw new Error("File too large. Maximum size: 10MB");
  ...
  /** Create via URL (existing method) */
  create: (data: CreateListingImageRequest): Promise<ApiListingImage> =>
    api.post<ApiListingImage>("/listing-images", data),

  update: (id: string, data: Partial<CreateListingImageRequest>): Promise<ApiListingImage> =>
    api.put<ApiListingImage>(`/listing-images/${id}`, data),

  delete: (id: string): Promise<void> => api.delete<void>(`/listing-images/${id}`),
};
